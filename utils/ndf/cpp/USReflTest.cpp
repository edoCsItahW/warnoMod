#ifdef __clang__
    #include "include/USRefl_clang.h"
#else
    #include "include/USRefl.h"
#endif
...
template<>
struct Ubpa::refl::TypeInfo<Point> : TypeInfoBase<Point> {
...
template<typename T>
struct Data {
    [[range(std::pair<T, T>{static_cast<T>(0), static_cast<T>(100)})]] T value;
};

template<typename T>
struct Ubpa::refl::TypeInfo<Data<T>> : TypeInfoBase<Data<T>> {
    // [!] all instance types have the same name
    static constexpr char name[5]     = "data";
    static constexpr AttrList attrs   = {};
    static constexpr FieldList fields = {
        Field{
              TSTR("value"), &Data<T>::value,
              AttrList{
...
template<>
struct Ubpa::refl::TypeInfo<A> : TypeInfoBase<A> {
...
template<>
struct Ubpa::refl::TypeInfo<B> : TypeInfoBase<B, Base<A>> {
...
template<>
struct Ubpa::refl::TypeInfo<C> : TypeInfoBase<C, Base<A>> {
...
template<>
struct Ubpa::refl::TypeInfo<D> : TypeInfoBase<D, Base<B>, Base<C>> {
...
void test_inheritance() {
    cout << "====================" << endl << " inheritance" << endl << "====================" << endl;
    ...
    cout << "[DFS]" << endl;
    TypeInfo<D>::DFS_ForEach([](auto t, std::size_t) { t.fields.forEach([](auto field) { cout << field.name << endl; }); });
...
enum class [[enum_attr("enum_attr_value")]] Color{RED [[enumerator_attr("enumerator_attr_value"), func(&Func<1>)]] = 1, GREEN [[func(&Func<2>)]] = 2, BLUE [[func(&Func<3>)]] = 4};

template<>
struct Ubpa::refl::TypeInfo<Color> : TypeInfoBase<Color> {
...
void test_enum() {
    cout << "====================" << endl << " enum" << endl << "====================" << endl;
    cout << TypeInfo<Color>::name << endl;

    TypeInfo<Color>::fields.forEach([](auto field) { cout << field.name << endl; });
...
template<>
struct Ubpa::refl::TypeInfo<FuncList> : TypeInfoBase<FuncList> {
    static constexpr char name[9]     = "FuncList";
    static constexpr AttrList attrs   = {};
    static constexpr FieldList fields = {
        Field{TSTR("Func0"), &Type::Func0},
        Field{
              TSTR("Func1"), &Type::Func1,
              AttrList{
                Attr{TSTR("default_functions"), std::tuple{[](Type* this_) { return this_->Func1(); }}},
            }
        },
    };
};
...
void test_function() {
    ...
    TypeInfo<FuncList>::fields.forEach([](auto field) { cout << field.name << endl; });
}
...
template<>
struct Ubpa::refl::TypeInfo<VA> : TypeInfoBase<VA> {
...
template<>
struct Ubpa::refl::TypeInfo<VB> : TypeInfoBase<VB, Base<VA, true>> {
...
template<>
struct Ubpa::refl::TypeInfo<VC> : TypeInfoBase<VC, Base<VA, true>> {
...
template<>
struct Ubpa::refl::TypeInfo<VD> : TypeInfoBase<VD, Base<VB>, Base<VC>> {
...
void test_virtual() {
    ...
    cout << "[field]" << endl;
    TypeInfo<VD>::DFS_ForEach([](auto t, std::size_t) { t.fields.forEach([](auto field) { cout << field.name << endl; }); });
